...
namespace grpcmmo::client
{
    namespace
    {
        ...
        bool NearlyEqualDirection(const glm::vec3& lhs, const glm::vec3& rhs)
        {
            ...
            if (lhs_length_squared <= 0.000001f ||
        ...
        [[nodiscard]] glm::dvec3 ToDVec3(
            const grpcmmo::world::v1::Vector3d& value
        )
        {
            return glm::dvec3(value.x(), value.y(), value.z());
        }

        [[nodiscard]] glm::dquat ToDQuat(
            const grpcmmo::world::v1::Quaterniond& value
        )
        {
        ...
        )
        {
            const glm::dvec3 world_forward = glm::rotate(
                ToDQuat(patch.transform().orientation()),
    ...
    void ClientWorld::Init()
    {
        ...
        gameplay_camera_ =
            &camera_director_.CreateCamera("gameplay_follow_camera");
    ...
    void ClientWorld::Tick(float delta_seconds)
    {
        ...
        const CameraBoon* active_boon =
            player_controller_ != nullptr ? &player_controller_->GetCameraBoon()
                                          : nullptr;
    ...
    PlayerController::FrameInput ClientWorld::BuildFrameInput() const
    {
        PlayerController::FrameInput frame_input =
            player_controller_ != nullptr
                ? player_controller_->ConsumeFrameInput()
    ...
    glm::vec3 ClientWorld::GroundRenderPosition(const glm::vec3& render_position
    ) const
    {
        return terrain_sampler_.GroundLocalPosition(render_position);
    }

    void ClientWorld::AdvanceControlledPrediction(
    ...
    void ClientWorld::ApplyEntityPatch(
        const grpcmmo::world::v1::EntityPatch& patch
    )
    {
        ...
        if (it == replicated_actors_.end())
        {
            ...
            replicated_actors_.emplace(
                patch.entity_id(), std::move(created_actor)
            );
        }
        else
        {
