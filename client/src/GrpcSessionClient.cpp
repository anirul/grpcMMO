...
namespace grpcmmo::client
{
    ...
    )
    {
        ...
        if (!login_status.ok())
        {
            ...
            {
                *error_message =
                    "login failed: " + login_status.error_message();
            }
            return false;
        }
    ...
    bool GrpcSessionClient::SendMove(const MoveCommand& move_command)
    {
        ...
        const auto sequence =
            next_input_sequence_.fetch_add(1, std::memory_order_relaxed);
    ...
    )
    {
        ...
        const grpc::Status list_status = auth_stub->ListCharacters(
        ...
        if (!list_status.ok())
        {
            ...
            {
                *error_message =
                    "list characters failed: " + list_status.error_message();
            }
            return false;
        }
        ...
        if (!create_status.ok())
        {
            ...
            {
                *error_message =
                    "create character failed: " + create_status.error_message();
            }
            return false;
        }
        ...
        writer_thread_ = std::thread([this]()
        {
            for (;;)
            {
                grpcmmo::session::v1::ClientMessage message;
                {
                    ...
                    {
                        if (writer_stop_requested_.load(
                                std::memory_order_relaxed
                            ))
                        {
                            break;
