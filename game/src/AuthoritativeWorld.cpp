...
namespace grpcmmo::game
{
    namespace
    {
        ...
        )
        {
            return ProjectVectorOntoTangent(
                LocalDirectionToWorld(local_vector, PreviewPatchFrame()),
                up_unit
            );
        }
        ...
        )
        {
            ...
            const glm::dvec3 side = NormalizeOrFallback(
                glm::cross(forward, up), glm::dvec3(0.0, 0.0, 1.0)
            );
        ...
        [[nodiscard]] glm::dvec3 BuildSpawnPosition(
            ...
            const grpcmmo::shared::planet::PreviewPatchTerrainSampler&
                terrain_sampler
        )
        {
        ...
        [[nodiscard]] bool NearlyChanged(
            const glm::dvec3& lhs, const glm::dvec3& rhs
        )
        {
        ...
        )
        {
            ...
            if (requested_distance <= max_distance_m ||
                requested_distance <= 0.000001)
            {
    ...
    grpcmmo::world::v1::ReplicationBatch AuthoritativeWorld::
        MakeReplicationBatch(
            const PlayerState& player_state,
            std::uint64_t server_time_ms,
            std::uint64_t server_tick,
