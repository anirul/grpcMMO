    ...
    ) override
    {
        ...
        if (!stream->Read(&first_message))
        {
            std::cout << "[game] open_session failed: missing begin_session"
                      << std::endl;
            return grpc::Status(
                grpc::StatusCode::FAILED_PRECONDITION, "begin_session required"
            );
        }
        ...
        ready->set_interpolation_back_time_ms_hint(interpolation_back_time_ms_);
        ready->set_heartbeat_interval_ms_hint(heartbeat_interval_ms_);
        *ready->mutable_initial_controlled_entity() = connection.initial_entity;
        stream->Write(ready_message);

        grpcmmo::session::v1::ServerMessage initial_replication;
        *initial_replication.mutable_replication() = connection.initial_batch;
