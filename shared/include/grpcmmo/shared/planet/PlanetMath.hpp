...
[[nodiscard]] inline glm::dvec3 NormalizeOrFallback(const glm::dvec3& value,
...
{
    return NormalizeOrFallback(planet_centered_position_m,
...
[[nodiscard]] inline TangentFrame BuildTangentFrameFromUp(const glm::dvec3& up_unit)
{
    const glm::dvec3 up = NormalizeOrFallback(up_unit, glm::dvec3(0.0, 1.0, 0.0));
    ...
    frame.up = up;
    frame.east =
        NormalizeOrFallback(glm::cross(reference_axis, up), glm::dvec3(1.0, 0.0, 0.0));
    frame.north =
        NormalizeOrFallback(glm::cross(up, frame.east), glm::dvec3(0.0, 0.0, 1.0));
    return frame;
}

[[nodiscard]] inline glm::dvec3 ProjectVectorOntoTangent(const glm::dvec3& vector,
...
[[nodiscard]] inline glm::dvec3 TangentOffsetToPlanetSpace(
    ...
    double east_offset_m,
    double north_offset_m)
{
    return (frame.east * east_offset_m) + (frame.north * north_offset_m);
}

[[nodiscard]] inline glm::dvec3 DirectionFromLatLonDegrees(double latitude_deg,
...
[[nodiscard]] inline glm::dvec3 PositionFromLatLonAltitude(double planet_radius_m,
                                                           double latitude_deg,
                                                           double longitude_deg,
                                                           double altitude_m)
{
...
[[nodiscard]] inline glm::dvec3 WorldPositionToLocal(
    ...
    const TangentFrame& frame)
{
    const glm::dvec3 offset = planet_position_m - local_origin_planet_position_m;
    return glm::dvec3(glm::dot(offset, frame.east),
                      glm::dot(offset, frame.up),
                      glm::dot(offset, frame.north));
}

[[nodiscard]] inline glm::dvec3 WorldDirectionToLocal(const glm::dvec3& world_direction,
                                                      const TangentFrame& frame)
{
    return glm::dvec3(glm::dot(world_direction, frame.east),
                      glm::dot(world_direction, frame.up),
                      glm::dot(world_direction, frame.north));
}

[[nodiscard]] inline glm::dvec3 LocalDirectionToWorld(const glm::dvec3& local_direction,
                                                      const TangentFrame& frame)
{
    return (frame.east * local_direction.x) +
           (frame.up * local_direction.y) +
    ...
    double altitude_m)
{
    ...
    distances.line_of_sight_m =
        std::sqrt(clamped_altitude * ((2.0 * clamped_radius) + clamped_altitude));
    ...
    const double cosine =
        std::clamp(clamped_radius / (clamped_radius + clamped_altitude), -1.0, 1.0);
