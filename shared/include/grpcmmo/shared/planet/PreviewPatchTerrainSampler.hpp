...
{
public:
    [[nodiscard]] static double SampleTriangleSplitCellHeight(float north_west_height_m,
        ...
        if ((clamped_row_t + clamped_col_t) <= 1.0)
        {
            return static_cast<double>(north_west_height_m) +
                   ((static_cast<double>(north_east_height_m) -
                     static_cast<double>(north_west_height_m)) * clamped_col_t) +
                   ((static_cast<double>(south_west_height_m) -
                     static_cast<double>(north_west_height_m)) * clamped_row_t);
        }
    ...
    [[nodiscard]] bool Load(const PreviewPatchConfig& config = kMarsPreviewPatch000)
    {
        Reset();
        config_ = config;
        ...
        try
        {
            const std::filesystem::path patch_directory =
                (std::filesystem::path(grpcmmo::shared::kDataRoot) /
                 ...
                 config_.patch_id)
                    .lexically_normal();
            const std::filesystem::path metadata_path = patch_directory / "patch.json";
            const std::filesystem::path height_path = patch_directory / "ground_heights.f32";
            ...
            source_planet_radius_m_ = ExtractJsonNumber(metadata, "planet_radius_m");
            source_height_scale_ = ExtractJsonNumber(metadata, "height_scale");
            source_origin_height_m_ = ExtractJsonNumber(metadata, "origin_height_m");
            ...
            relative_heights_m_ = ReadRelativeHeights(height_path, rows_, cols_);

            loaded_ = rows_ > 0u && cols_ > 0u && !relative_heights_m_.empty();
            return loaded_;
        }
        catch (...)
        {
    ...
    [[nodiscard]] glm::dvec3 GroundWorldPosition(
        const glm::dvec3& approx_world_position) const
    {
        return GroundWorldPosition(approx_world_position, config_.planet_radius_m);
    }

    [[nodiscard]] glm::dvec3 GroundWorldPosition(
        const glm::dvec3& approx_world_position,
        double planet_radius_m) const
    {
        return ProjectToAltitude(approx_world_position,
                                 planet_radius_m,
    ...
    {
        const glm::dvec3 local_position_d(local_position.x,
                                          local_position.y,
                                          local_position.z);
        const glm::dvec3 approx_world_position =
            preview_patch_origin_planet_position_m_ +
            LocalDirectionToWorld(local_position_d,
                                  TangentFrame{preview_patch_frame_east_,
        ...
        const glm::dvec3 grounded_local_position =
            WorldPositionToLocal(grounded_world_position,
                                 preview_patch_origin_planet_position_m_,
                                 TangentFrame{preview_patch_frame_east_,
                                              preview_patch_frame_north_,
                                              preview_patch_frame_up_});
        return glm::vec3(static_cast<float>(grounded_local_position.x),
...
private:
    ...
    [[nodiscard]] static double ExtractJsonNumber(const std::string& text,
    ...
    [[nodiscard]] double SampleRelativeHeightM(double latitude_deg,
                                               double longitude_deg) const
    {
        ...
        const auto index_of = [this](std::uint32_t sample_row,
                                     std::uint32_t sample_col)
        {
        ...
        return SampleTriangleSplitCellHeight(h00, h10, h01, h11, row_t, col_t);
    }
