...
class PreviewPatchTerrainSampler
{
  public:
    ...
    )
    {
        ...
        const double south_east_weight = (clamped_row_t + clamped_col_t) - 1.0;
        ...
        try
        {
            const std::filesystem::path patch_directory =
                (std::filesystem::path(grpcmmo::shared::kDataRoot) / "tiles" /
                 config_.planet_id / config_.patch_id)
                    .lexically_normal();
  ...
  private:
    ...
    ) const
    {
        ...
        const double row = Clamp01((max_lat_deg_ - latitude_deg) / lat_range) *
                           static_cast<double>(rows_ - 1u);
        const double col = Clamp01((longitude_deg - min_lon_deg_) / lon_range) *
                           static_cast<double>(cols_ - 1u);

        const std::uint32_t row0 = static_cast<std::uint32_t>(std::floor(row));
        const std::uint32_t col0 = static_cast<std::uint32_t>(std::floor(col));
