...
namespace grpcmmo::tools::terrain
{
    namespace
    {
        ...
        struct RasterWindow
        {
            ...
            [[nodiscard]] double At(std::uint32_t row, std::uint32_t col) const
            {
                const std::size_t local_row =
                    static_cast<std::size_t>(row - row_begin);
                const std::size_t local_col =
                    static_cast<std::size_t>(col - col_begin);
                return samples[(local_row * Width()) + local_col];
            }

            [[nodiscard]] double SampleBilinear(
                double source_col, double source_row
            ) const
            {
                ...
                const auto col0 =
                    static_cast<std::uint32_t>(std::floor(source_col));
                const auto row0 =
                    static_cast<std::uint32_t>(std::floor(source_row));
        ...
        )
        {
            ...
            {
                throw std::runtime_error(
                    "failed to open TIFF: " + input_tiff.string()
                );
            }
        ...
        [[nodiscard]] RasterMetadata ReadRasterMetadata(TIFF* handle)
        {
            ...
            if (TIFFGetField(handle, TIFFTAG_IMAGEWIDTH, &metadata.width) !=
                    1 ||
                TIFFGetField(handle, TIFFTAG_IMAGELENGTH, &metadata.height) !=
                    1)
            {
            ...
            TIFFGetFieldDefaulted(
                handle, TIFFTAG_SAMPLEFORMAT, &raw_sample_format
            );
            ...
            {
                throw std::runtime_error(
                    "only single-band TIFF rasters are supported"
                );
            }

            const bool supported_bits = metadata.bits_per_sample == 8u ||
                                        metadata.bits_per_sample == 16u ||
                                        metadata.bits_per_sample == 32u ||
                                        metadata.bits_per_sample == 64u;
            if (!supported_bits)
            {
        ...
        )
        {
            const double clamped_latitude =
                std::clamp(latitude_deg, -90.0, 90.0);
        ...
        )
        {
            const double clamped_longitude =
                std::clamp(longitude_deg, -180.0, 180.0);
        ...
        void ValidateBakeSettings(const BakeSettings& settings)
        {
            ...
            {
                throw std::runtime_error(
                    "lat_span_deg must be in the range (0, 180]"
                );
            }
            ...
            {
                throw std::runtime_error(
                    "lon_span_deg must be in the range (0, 360]"
                );
            }
        ...
        {
            const double source_row_a =
                SourceRowFromLatitude(metadata, max_lat_deg);
            const double source_row_b =
                SourceRowFromLatitude(metadata, min_lat_deg);
            const double source_col_a =
                SourceColFromLongitude(metadata, min_lon_deg);
            const double source_col_b =
                SourceColFromLongitude(metadata, max_lon_deg);
            ...
            for (std::uint32_t row = window.row_begin; row <= window.row_end;
                 ++row)
            {
                ...
                for (std::uint32_t col = window.col_begin;
                     col <= window.col_end;
                     ++col)
                {
        ...
        )
        {
            ...
            const auto tangent_frame =
                grpcmmo::shared::planet::BuildTangentFrameFromUp(
                    center_direction
                );
            const glm::dvec3 center_position =
                center_direction *
                (settings.planet_radius_m + scaled_origin_height_m);
            ...
            for (std::uint32_t row = 0; row < settings.output_rows; ++row)
            {
                ...
                {
                    const double longitude_deg = OutputLongitude(
                        settings, col, min_lon_deg, max_lon_deg
                    );
                    const std::size_t index =
                        static_cast<std::size_t>(row) * settings.output_cols +
                        col;
                    ...
                    const glm::dvec3 planet_position =
                        direction *
                        (settings.planet_radius_m + absolute_height_m);
                    const glm::dvec3 local_offset =
                        planet_position - center_position;
            ...
            for (std::uint32_t row = 0; row < rows; ++row)
            {
                for (std::uint32_t col = 0; col < cols; ++col)
                {
                    ...
                    Vec3 normal = Normalize(
                        Cross(Subtract(south, north), Subtract(right, left))
                    );
                    if (normal.y < 0.0)
                    {
                        normal.x = -normal.x;
            ...
            for (std::uint32_t row = 0; row < rows; ++row)
            {
                ...
                for (std::uint32_t col = 0; col < cols; ++col)
                {
                    const double u = cols > 1u
                                         ...
                                         : 0.0;
                    texcoords[static_cast<std::size_t>(row) * cols + col] =
                        Vec2{
            ...
            for (int y = 0; y < kPreviewTextureSizePx; ++y)
            {
                for (int x = 0; x < kPreviewTextureSizePx; ++x)
                {
                    const bool micro_checker =
                        (((x / kMicroCellSize) + (y / kMicroCellSize)) % 2) ==
                        0;
                    const bool minor_checker =
                        (((x / kMinorCellSize) + (y / kMinorCellSize)) % 2) ==
                        0;
        ...
        )
        {
            ...
            const auto texcoords = BuildVertexTexcoords(
                settings.output_rows, settings.output_cols
            );
            const auto indices = BuildTriangleIndices(
                settings.output_rows, settings.output_cols
            );
            ...
            output << "        \"baseColorFactor\": [1.000000, 1.000000, "
                      "1.000000, "
                      "1.000000],\n";
            ...
            output << "      \"type\": \"VEC3\",\n";
            output << "      \"max\": [" << max_position[0] << ", "
                   << max_position[1] << ", " << max_position[2] << "],\n";
            output << "      \"min\": [" << min_position[0] << ", "
                   << min_position[1] << ", " << min_position[2] << "]\n";
            output << "    },\n";
            output << "    {\n";
            output << "      \"bufferView\": 2,\n";
            output << "      \"componentType\": 5126,\n";
        ...
        )
        {
            ...
            output << "  \"planet_id\": \""
                   << EscapeJsonString(settings.planet_id) << "\",\n";
            output << "  \"patch_id\": \""
                   << EscapeJsonString(settings.patch_id) << "\",\n";
            output
                << "  \"source_tiff\": \""
                << EscapeJsonString(
            ...
            output << "    \"sample_format\": \""
                   << RasterSampleFormatName(
                          result.source_metadata.sample_format
                      )
                   << "\"\n";
            output << "  },\n";
            output << "  \"height_grid\": {\n";
            ...
            output << "    \"min_relative_height_m\": "
                   << result.min_relative_height_m << ",\n";
            output << "    \"max_relative_height_m\": "
                   << result.max_relative_height_m << "\n";
            output << "  },\n";
            output << "  \"preview_mesh\": {\n";
            output << "    \"file\": \"" << kPreviewGltfFileName << "\",\n";
            output << "    \"base_color_texture\": \""
                   << kPreviewTextureFileName << "\",\n";
    ...
    BakeResult BakeTerrainPatch(const BakeSettings& settings)
    {
        ...
        result.relative_heights_m.resize(
            static_cast<std::size_t>(settings.output_rows) *
        ...
        for (std::uint32_t row = 0; row < settings.output_rows; ++row)
        {
            ...
            const double source_row =
                SourceRowFromLatitude(metadata, latitude_deg);
            for (std::uint32_t col = 0; col < settings.output_cols; ++col)
            {
