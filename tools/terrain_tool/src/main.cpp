...
int main(int argc, char** argv)
{
    ...
    try
    {
        const std::filesystem::path input_tiff =
            absl::GetFlag(FLAGS_input_tiff);
        if (absl::GetFlag(FLAGS_inspect_only))
        {
            const auto metadata =
                grpcmmo::tools::terrain::InspectRaster(input_tiff);
            std::cout << "Raster metadata" << std::endl;
            std::cout << "  input_tiff: "
                      << std::filesystem::absolute(input_tiff).string()
                      << std::endl;
            std::cout << "  width: " << metadata.width << std::endl;
            std::cout << "  height: " << metadata.height << std::endl;
            std::cout << "  bits_per_sample: " << metadata.bits_per_sample
                      << std::endl;
            std::cout << "  samples_per_pixel: " << metadata.samples_per_pixel
                      << std::endl;
        ...
        std::cout << "  origin_height_m: " << result.origin_height_m
                  << std::endl;
        std::cout << "  relative_height_range_m: ["
                  << result.min_relative_height_m << ", "
                  << result.max_relative_height_m << "]" << std::endl;
        std::cout << "  height_file: " << result.height_file.string()
                  << std::endl;
        std::cout << "  preview_gltf: " << result.preview_gltf_file.string()
                  << std::endl;
        std::cout << "  metadata_file: " << result.metadata_file.string()
                  << std::endl;
        return 0;
    }
